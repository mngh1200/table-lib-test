 ...
 * [{name: 'column1', order:listtable.sort.ORDER_ASC}, {name: 'column2', order:listtable.sort.ORDER_DESC}]
...
listtable.sort.sortObject = function(data, sortkeys) {
  ...
  sortdata.sort(function(a, b) {
    ...
    for (var i = 0; i < keylen; i++) {
      ...
      if (sortkey.order === listtable.TYPE_ASC) {
        if (a[sortkey.keyname] < b[sortkey.keyname]) return -1;
        if (a[sortkey.keyname] > b[sortkey.keyname]) return 1;
      } else if(sortkey.order === listtable.TYPE_DESC) {
        if (a[sortkey.keyname] > b[sortkey.keyname]) return -1;
        if (a[sortkey.keyname] < b[sortkey.keyname]) return 1;
      }
    }
